...
const EventSource = _eventSourcePolyfill.NativeEventSource || _eventSourcePolyfill.EventSourcePolyfill;
...
function generatePiece(id, index) {
  ...
  image.addEventListener('click', function (e) {
    if (e.target instanceof HTMLImageElement && !dragging) {
      ...
      if (playing_as == 'white' && id > 0 || playing_as == 'black' && id < 0) {
        return;
      }
...
window.onload = event => {
  loadBoard(() => {
    board.forEach((id, index) => {
      ...
      document.getElementsByClassName('board')[0].getElementsByClassName('square')[index].appendChild(image);
    });
  });
  let squares = document.getElementsByClassName('square');
  for (var i = 0; i < 64; i++) {
    ...
    square.addEventListener('click', function (e) {
      if (dragging && index != draged_piece) {
        fetch('http://localhost:8080/chess/board/move/', {
          ...
          console.log(code);
          dragging = false;
          document.body.getElementsByClassName('moving-piece')[0].remove();
          updateBoard();
        }));
      }
    });
...
},{"event-source-polyfill":2}],2:[function(require,module,exports){
/** @license
 * eventsource.js
